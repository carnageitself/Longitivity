...
const FloatingDockMobile = ({
  ...
  return (
    <div className={cn("relative block md:hidden", className)}>
      ...
      >
        <IconLayoutNavbarCollapse className="h-5 w-5 text-muted" />
      </button>
    </div>
  );
};
...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn("hidden h-14 items-center gap-2 md:flex", className)}
    >
      {items.map((item) => (
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="absolute -top-8 left-1/2 w-fit rounded-md border border-border bg-surface px-2 py-0.5 text-xs whitespace-pre text-foreground"
            >
              {title}
            </motion.div>
          )}
