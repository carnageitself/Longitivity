...
export function PlaceholdersAndVanishInput({
  ...
  return (
    <form
      className={cn(
        "w-full relative max-w-xl mx-auto bg-surface border border-border h-12 rounded-full overflow-hidden transition duration-200",
        className,
      )}
    ...
    >
      ...
      <button
        disabled={!value}
        type="submit"
        className="absolute right-2 top-1/2 z-50 -translate-y-1/2 h-8 w-8 rounded-full bg-foreground disabled:opacity-40 transition duration-200 flex items-center justify-center"
      >
        <motion.svg
        ...
        <AnimatePresence mode="wait">
          {!value && (
            <motion.p
              ...
              className="text-sm sm:text-base font-normal text-muted pl-4 sm:pl-12 text-left w-[calc(100%-2rem)] truncate"
            >
